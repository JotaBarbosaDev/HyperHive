...
const toastStyle = tva({
  base: 'p-4 m-1 rounded-md gap-1 web:pointer-events-none shadow-hard-5 border-outline-100',
  variants: {
    action: {
      error: 'bg-error-800',
...
>(function ToastTitle({ className, size = 'md', children, ...props }, ref) {
  ...
  React.useEffect(() => {
    ...
    const toAccessibleText = (value: React.ReactNode): string | null => {
      ...
      if (Array.isArray(value)) {
        const parts = value
          .map(toAccessibleText)
          .filter((part): part is string => Boolean(part));
        return parts.length > 0 ? parts.join(' ') : null;
      }
      return null;
    };
    const accessibleText = toAccessibleText(children);
    if (accessibleText) {
